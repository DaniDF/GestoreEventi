...
public class MyGridPaneView extends GridPane implements EventHandler<ActionEvent> {
	...
	public MyGridPaneView(Controller controller) {
		super();
		...
		this.add(this.getNextEventRemaning(), 2, 1);
	...
	private Node getPregress(Duration d) {
	...
	private Node getNextEventRemaning() {
		VBox result = new VBox();
		Text desc = new Text();
		Optional<Event> nextEvent = this.controller.getNextEvent();
		Duration timeRemaning = Duration.between(ZonedDateTime.now(),nextEvent.get().getStart());
		...
		result.getChildren().add(this.getPregress(timeRemaning));
